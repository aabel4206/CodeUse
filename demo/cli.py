...
def main() -> int:
    ...
    parser.add_argument(
        "--no-follow",
        action="store_true",
        help="Disable the follow-one-link behaviour",
    )
    ...
    if args.executor_json:
        ...
        raw = run_audit(
            target_url=args.url,
            run_id=None,
            slow_ms=max(int(args.slow_ms or 0), 0),
            follow_one_link=not args.no_follow,
        )
    ...
    if isinstance(raw, dict):
        child_url = ((raw.get("extras") or {}).get("child") or {}).get("url")
    ...
    print("Followed link:", child_url or "none")
